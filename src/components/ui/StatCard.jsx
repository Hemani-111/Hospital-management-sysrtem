  ...
  return (
    <div className="glass-morphism rounded-[2.5rem] p-6 flex items-center gap-6 border border-white/40 dark:border-slate-800/40 shadow-premium hover:shadow-glow transition-all duration-500 hover:-translate-y-1 hover:bg-white/80 dark:hover:bg-slate-900/80 cursor-pointer group grow">
      <div className="relative">
        <div className={`size-12 sm:size-14 rounded-2xl bg-${color}-100/80 dark:bg-${color}-900/30 flex items-center justify-center text-${color}-600 shadow-inner group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 ring-1 ring-${color}-200/50 dark:ring-${color}-800/50`}>
          <span className="material-symbols-outlined text-2xl sm:text-3xl transition-transform duration-500 group-hover:scale-110">{icon}</span>
        </div>
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-2xl sm:text-3xl font-display font-black text-slate-900 dark:text-slate-100 leading-none">{value}</h3>
          {trend && (
