...
const DoctorDashboard = () => {
  ...
  const { data: todayAppts, isLoading: loadAppts } = useQuery({
  ...
  const { data: deptCases, isLoading: loadCases } = useQuery({
      ...
      <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
        ...
        <section className="bg-rose-50/50 dark:bg-rose-900/10 rounded-[2.5rem] p-8 border border-rose-100/50 dark:border-rose-800/20 shadow-sm overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 gap-4 relative z-10">
            {loadCases ? (
              <Skeleton variant="table-row" className="bg-white/50 dark:bg-slate-800/50 rounded-3xl" />
            ) : (deptCases || []).length === 0 ? (
              <div className="text-center py-6 text-slate-400 font-bold italic">No emergency alerts for your department.</div>
            ) : (
        ...
        <section className="glass-morphism rounded-[2.5rem] border border-white/20 dark:border-slate-800/20 shadow-premium overflow-hidden">
          ...
          <div className="p-6 md:p-8 space-y-6">
            {loadAppts ? (
              [...Array(2)].map((_, i) => (
                <Skeleton key={i} variant="table-row" className="bg-white/40 dark:bg-slate-800/20 rounded-3xl h-24" />
              ))
            ) : todayAppts?.length === 0 ? (
               <EmptyState 
                 title="No Appointments Today" 
                 description="You have a clear schedule for today. Use this time for research or patient follow-ups."
